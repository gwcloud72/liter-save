...
function BrandMark({ brand }) {
  const name = String(brand || '').toUpperCase();
  const isSk = name.includes('SK');
  const isGs = name.includes('GS');
  const isSoil = name.includes('S-OIL') || name.includes('S-OIL');
  const isHyundai = name.includes('현대') || name.includes('HD');
  const tone = isSk ? 'bg-red-500' : isGs ? 'bg-emerald-500' : isSoil ? 'bg-yellow-400' : isHyundai ? 'bg-blue-500' : 'bg-slate-400';
  const text = isSk ? 'SK' : isGs ? 'GS' : isSoil ? 'S' : isHyundai ? 'H' : 'O';
  return <span className={`grid size-4 shrink-0 place-items-center rounded-full ${tone} text-[8px] font-black leading-none text-white ring-2 ring-white`} aria-hidden="true">{text}</span>;
}
...
function StationRow({ station, rank, averagePrice, favorites }) {
  ...
  return (
    <tr className="hover:bg-slate-50/80">
      <td className="px-3 py-[12px]"><span className={`grid size-6 place-items-center rounded-full border text-xs font-bold ${rankClass(rank)}`}>{rank}</span></td>
      <td className="min-w-0 px-3 py-[12px] font-bold text-slate-900"><p className="line-clamp-2 min-w-0">{station.name}</p></td>
      <td className="whitespace-nowrap px-3 py-[12px] text-[17px] font-extrabold tabular-nums text-blue-600">{formatNumber(station.price)}</td>
      <td className={`whitespace-nowrap px-3 py-[12px] font-bold tabular-nums ${saving <= 0 ? 'text-blue-600' : 'text-red-500'}`}>{formatSignedWon(saving)}</td>
      <td className="px-3 py-[12px] font-extrabold text-slate-700"><span className="inline-flex min-w-0 items-center gap-2"><BrandMark brand={station.brand} /><span className={`truncate ${station.brandTone}`}>{station.brand}</span></span></td>
      <td className="min-w-0 px-3 py-[12px] font-medium text-slate-500"><p className="line-clamp-2">{station.address}</p></td>
      <td className="px-3 py-[12px] text-center"><div className="flex justify-center gap-2 text-[11px] font-bold text-slate-600"><a href={mapUrl} target="_blank" rel="noopener noreferrer">↗ 길찾기</a><button type="button" onClick={() => favorites?.toggle(id)} aria-label={`${station.name} 관심 주유소 토글`} aria-pressed={isFavorite}>{isFavorite ? '저장됨' : '관심'}</button></div></td>
    </tr>
  );
}
  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-start gap-2">
            ...
            <p className="line-clamp-2 min-w-0 font-extrabold text-slate-950">{station.name}</p>
          </div>
          <p className="mt-2 inline-flex max-w-full items-center gap-2 text-sm font-extrabold"><BrandMark brand={station.brand} /><span className={`truncate ${station.brandTone}`}>{station.brand}</span></p>
          <p className="mt-1 line-clamp-2 text-xs font-medium text-slate-500">{station.address}</p>
        </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 text-xs font-extrabold">
        <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="grid h-9 place-items-center rounded-xl border border-slate-200 text-slate-700">↗ 길찾기</a>
...
export default function StationTable({ stations, averagePrice, favorites, title = '서울특별시 휘발유 가격 비교', reset, canReset, emptyMessage, emptyAction, emptyActionLabel }) {
  ...
  return (
    <section className="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:p-5">
      <div className="mb-3 flex items-center gap-2 md:mb-4"><h2 className="text-[16px] font-extrabold tracking-tight text-slate-950 md:text-lg">{title}</h2><span className="text-slate-400">ⓘ</span></div>
      {!stations.length ? <EmptyState onReset={emptyHandler} message={emptyMessage} actionLabel={label} /> : <>
        <div className="hidden overflow-hidden rounded-xl border border-slate-200 md:block">
          <table className="w-full table-fixed border-collapse text-left text-[13px]"><caption className="sr-only">저가 주유소 목록</caption><thead className="bg-slate-50 text-[12px] font-bold text-slate-500"><tr><th className="w-12 px-3 py-2">순위</th><th className="w-[21%] px-3 py-2">주유소</th><th className="w-[12%] px-3 py-2">가격</th><th className="w-[12%] px-3 py-2">평균 대비</th><th className="w-[14%] px-3 py-2">브랜드</th><th className="w-[27%] px-3 py-2">주소</th><th className="w-[14%] px-3 py-2 text-center">관심</th></tr></thead><tbody className="divide-y divide-slate-100">{visibleStations.map((station, index) => <StationRow key={getStationKey(station)} station={station} rank={index + 1} averagePrice={averagePrice} favorites={favorites} />)}</tbody></table>
        </div>
