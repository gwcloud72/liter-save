...
export function KakaoMapPanel({ stations, onSelect, tall = false }: { stations: Station[]; onSelect: (station: Station) => void; tall?: boolean }) {
  ...
  return <Card padding="normal" className={`relative overflow-hidden bg-primary-50 ${tall ? 'min-h-map' : 'min-h-hero'}`}>
    ...
    {best ? <button type="button" onClick={()=>onSelect(best)} className="relative mt-4 flex w-full items-center justify-between rounded-lg border border-primary-100 bg-white px-4 py-3 text-left shadow-card hover:border-primary-500"><div><p className="text-[11px] text-primary-600">선택 추천</p><p className="mt-ds-0.5 truncate text-[15px] text-ink-900">{best.name}</p><p className="mt-ds-0.5 text-[13px] text-ink-500">{best.brand} · {formatDistanceKm(best.distance)}</p></div><strong className="inline-flex items-baseline text-[20px] font-bold text-primary-500 tabular"><span>{best.price.toLocaleString()}</span><span className="v6-unit">원/L</span></strong></button> : null}
    {best ? <a href={kakaoRouteHref(best)} target="_blank" rel="noopener noreferrer" className="relative mt-3 inline-flex h-10 w-full items-center justify-center gap-2 rounded-md bg-primary-600 text-[15px] font-bold text-white hover:bg-primary-700">카카오맵 길찾기<ExternalLink size={15}/></a> : null}
  </Card>;
}
...
export function StationCard({ station, favorite = false, onToggle, onRoute }: { station: Station; favorite?: boolean; onToggle: () => void; onRoute?: () => void }) { return <Card padding="normal"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate font-bold text-ink-900">{station.name}</h3><p className="mt-1 truncate text-sm text-ink-500">{station.brand} · {formatDistanceKm(station.distance)}</p></div><button type="button" onClick={onToggle} aria-pressed={favorite} className={`shrink-0 rounded-full p-2 ${favorite ? 'bg-primary-100 text-primary-600' : 'bg-ink-100 text-ink-400 hover:text-primary-600'}`}><Star size={17} fill={favorite?'currentColor':'none'}/></button></div><div className="mt-5 grid grid-cols-trend-110 items-end gap-4"><div><strong className="inline-flex items-baseline gap-0.5 text-2xl font-bold text-primary-500 tabular"><span>{station.price.toLocaleString()}</span><span className="v6-unit">원/L</span></strong><p className="mt-ds-0.5 text-[13px] text-ink-500">{priceDiffCopy(station.avgDiff)}</p></div><MiniTrend values={station.trend} direction="down"/></div><a href={kakaoRouteHref(station)} target="_blank" rel="noopener noreferrer" onClick={onRoute} className="mt-4 inline-flex h-10 w-full items-center justify-center gap-2 rounded-md border border-ink-200 text-[15px] font-bold text-primary-600 hover:border-primary-500 hover:bg-primary-50">길찾기<ExternalLink size={15}/></a></Card>; }
