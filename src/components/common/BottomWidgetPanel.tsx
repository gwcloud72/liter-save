      ...
      {widgets.map((widget) => (
        <Card key={widget.title} padding={compact ? 'compact' : 'normal'} interactive>
          <div className="mb-ds-2 flex items-center justify-between gap-ds-2">
            ...
            <button type="button" onClick={() => onAction(widget.action)} className="shrink-0 rounded-sm px-ds-1 py-ds-0.5 text-caption font-bold text-ink-700 hover:bg-ink-100 hover:underline focus-visible:outline-none focus-visible:shadow-focus">{widget.action}</button>
          </div>
          <ul className={compact ? 'space-y-ds-1' : 'space-y-ds-1'}>
            {widget.items.slice(0, compact ? 3 : 4).map((item) => (
              <li key={item}>
                <button type="button" onClick={() => onAction(item)} className="flex w-full items-center justify-between gap-ds-1 rounded-md bg-ink-50 px-ds-2 py-ds-1.5 text-left text-caption text-ink-700 transition-fast duration-fast hover:bg-ink-100 focus-visible:outline-none focus-visible:shadow-focus">
