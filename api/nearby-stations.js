...
function getQuery(request) { if (request.query) return request.query; const requestUrl = new URL(request.url, 'https://request.invalid'); return Object.fromEntries(requestUrl.searchParams.entries()); }
function toNumber(value) { const numericValue = Number(value); return Number.isFinite(numericValue) ? numericValue : null; }
...
function normalizeStation(item) {
  const katecCoordinateX = toNumber(item.GIS_X_COOR ?? item.gisX ?? item.x);
  const katecCoordinateY = toNumber(item.GIS_Y_COOR ?? item.gisY ?? item.y);
  let latitude = null; let longitude = null;
  if (Number.isFinite(katecCoordinateX) && Number.isFinite(katecCoordinateY)) { try { const [lng, lat] = proj4(KATEC_CRS, WGS84_CRS, [katecCoordinateX, katecCoordinateY]); latitude = Number.isFinite(lat) ? Number(lat.toFixed(7)) : null; longitude = Number.isFinite(lng) ? Number(lng.toFixed(7)) : null; } catch { latitude = null; longitude = null; } }
...
async function fetchNearby({ lat, lng, fuel, sort }) {
  ...
  const [katecCoordinateX, katecCoordinateY] = proj4(WGS84_CRS, KATEC_CRS, [longitude, latitude]);
  const url = new URL(`${API_BASE}/aroundAll.do`);
  url.searchParams.set('out', 'json'); url.searchParams.set('x', String(katecCoordinateX)); url.searchParams.set('y', String(katecCoordinateY)); url.searchParams.set('radius', '5000'); url.searchParams.set('prodcd', FUEL_CODES[fuel] || FUEL_CODES['휘발유']); url.searchParams.set('sort', sort === 'price' ? '1' : '2'); url.searchParams.set('certkey', API_KEY);
...
export default async function handler(request, response) {
  try { const query = getQuery(request); const stations = await fetchNearby(query); const payload = { mode: 'opinet-aroundAll', radius: 5000, generatedAt: new Date().toISOString(), stations }; if (response?.status) return response.status(200).json(payload); return new Response(JSON.stringify(payload), { status: 200, headers: { 'content-type': 'application/json; charset=utf-8' } }); }
  catch (error) { const payload = { error: error.message }; if (response?.status) return response.status(500).json(payload); return new Response(JSON.stringify(payload), { status: 500, headers: { 'content-type': 'application/json; charset=utf-8' } }); }
}
